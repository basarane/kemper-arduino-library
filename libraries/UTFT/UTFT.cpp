	...
	for(j=0;j<cfont.y_size;j++) 
	{
		for (int zz=0; zz<(cfont.x_size/8); zz++)
		{
			...
			{   
				...
				if( (ch&(1<<(7-i)))!=0)   
				{
					if (cosDeg == UNDEFINED_COS)
					{
						newx=x+(((i+(zz*8)+(pos*cfont.x_size))*fcosDeg)-((j)*fsinDeg));
						newy=y+(((j)*fcosDeg)+((i+(zz*8)+(pos*cfont.x_size))*fsinDeg));
					}
					else
					{
						newx=x+(((i+(zz*8)+(pos*cfont.x_size))*cosDeg)-((j)*sinDeg));
						newy=y+(((j)*cosDeg)+((i+(zz*8)+(pos*cfont.x_size))*sinDeg));
					}
