...
Color ColorNone = {0xCC, 0xCC, 0xCC};
Color ColorWhite = {0xFF, 0xFF, 0xFF};
Color ColorOrange = {0xFF, 0xA5, 0x00};
Color ColorYellow = {0xFF, 0xFF, 0x00};
Color ColorCyan = {0x00, 0xFF, 0xFF};
Color ColorBlue = {0x00, 0x00, 0xFF};
Color ColorPurple = {0x80, 0x00, 0x80};
Color ColorGreen = {0x00, 0xFF, 0x00};
Color ColorLightGreen = {0xA0, 0xFF, 0xA0};
Color ColorRed = {0xFF, 0x00, 0x00};

StompInfo KEMPER_NAMESPACE::AllStomps[] = {
...
StompInfo* KEMPER_NAMESPACE::getStompInfo(int stompId, int id) {
	int indexOffset = stompId >= 6?100*stompId:0;
	for (int i=0;i<AllStompsCount;i++) {
		if (AllStomps[i].type == id + indexOffset)
			return &(AllStomps[i]);
	}
	return 0;
}
